...
public class FilterToSearchQuery {
    ...
    private SearchQuery convertCondition(FilterCondition filter) {
        ...
        filter.getMaxSize().ifPresent(maxSize -> searchQuery.andCriteria(SearchQuery.sizeLessThan(maxSize.asInt())));
        filter.getMinSize().ifPresent(minSize -> searchQuery.andCriteria(SearchQuery.sizeGreaterThan(minSize.asInt())));
