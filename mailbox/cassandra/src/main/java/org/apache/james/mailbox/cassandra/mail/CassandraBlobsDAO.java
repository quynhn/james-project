...
public class CassandraBlobsDAO {
    ...
    public CompletableFuture<UUID> save(byte[] data) {
    ...
    public InputStream read(UUID id) {
        ImmutableMap<Long, UUID> partIds = CassandraUtils.convertToStream(readBlob(id)
            .join())
    ...
    private CompletableFuture<ResultSet> readBlob(UUID blobId) {
    ...
    private CompletableFuture<Optional<Row>> readPart(UUID partId) {
        System.out.println("PartID is null, WHYYYYYY?");
        if (partId == null) {
    ...
    private UUID writePart(ByteBuffer data) {
        ...
        System.out.println("Saved PartID: " + partId);
        return partId;
    }

    private Stream<Pair<Integer, UUID>> computeBlobParts(byte[] data) {
    ...
    private Stream<Pair<Integer, UUID>> computeFinalByteBuffer(byte[] data, int offset, int index) {
