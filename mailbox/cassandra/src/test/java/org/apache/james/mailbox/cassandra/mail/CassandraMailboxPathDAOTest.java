...
public class CassandraMailboxPathDAOTest {
    ...
    public void saveShouldInsertNewEntry() throws Exception {
        ...
        Optional<CassandraId> cassandraId = testee.retrieveId(USER_INBOX_MAILBOXPATH).join();
        assertThat(cassandraId.get()).isEqualTo(INBOX_ID);
    }
    ...
    public void getUserMailboxesShouldReturnAllMailboxesOfUser() throws Exception {
        ...
        List<CassandraId> cassandraIds = testee.listUserMailboxes(USER_INBOX_MAILBOXPATH.getNamespace(), USER_INBOX_MAILBOXPATH.getUser())
            .join()
            .collect(Guavate.toImmutableList());

        assertThat(cassandraIds)
            .hasSize(2)
            .containsOnly(INBOX_ID, OUTBOX_ID);
    }
